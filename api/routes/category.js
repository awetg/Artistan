...
router.post('/', db.User.isLoggedIn,upload.single(), db.Category.addCategory);

/*update a category with authentication at PATCH: base_url/api/catefory/:category_id  */
router.patch('/:category_id', db.User.isLoggedIn,upload.single(), db.Category.updateCategory);

/* delete category by category id with authentication at DELETE: base_url/api/category/:category_id */
router.delete('/:category_id', db.User.isLoggedIn, db.Category.deleteCategory);
