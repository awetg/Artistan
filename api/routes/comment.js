...
router.post('/:post_id', db.User.isLoggedIn,upload.single(), db.Comment.createComment);

/*update a comment with authentication at PATCH: base_url/api/comment/:comment_id  */
router.patch('/:comment_id', db.User.isLoggedIn,upload.single(), db.Comment.updateComment);

/* delete comment by post id at DELETE: base_url/api/comment/:comment_id */
router.delete('/:comment_id', db.User.isLoggedIn, db.Comment.deleteComment);
