module.exports = (connection) => {
	...
	module.search = async(req, res) => {
		if (req.query.keywordTitle) {
			try {
				const [r, _] = await connection.query('SELECT * FROM post INNER JOIN media ON media.media_id=post.media INNER JOIN user ON user.user_id=post.owner WHERE title=?', [req.query.keywordTitle]);
				console.log(r);
				res.send(r);
			} catch (error) {
